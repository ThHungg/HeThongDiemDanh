...
const StudentAdvisorTable = ({ searchValue = "" }: StudentsTableProps) => {
  ...
  const handleSelectAll = () => {
    if (
      ...
      const allMsvs = paginatedStudents.map((s: any) => s.maSinhVien);
  ...
  const selectedStudentList = Array.from(selectedStudents)
    .map((msv) => {
      ...
      const fullStudent = paginatedStudents.find(
        (s: any) => s.maSinhVien === msv,
      );
      return {
        msv: cached.msv,
        name: cached.name,
        ...
        <table className="w-full border-collapse border border-gray-200">
          <thead className="bg-[#F8FAFC] text-[14px] text-[#64748B] border-b border-gray-200">
            <tr>
              ...
              <th className="text-center px-4 py-3 font-semibold w-12">STT</th>
            ...
            {paginatedStudents.length > 0 ? (
              paginatedStudents.map((student: any, index: number) => (
                ...
                >
                  ...
                  {/* STT - Số thứ tự */}
                  <td className="text-center px-4 py-2 font-semibold text-[#111827]">
...
export default memo(StudentAdvisorTable);
